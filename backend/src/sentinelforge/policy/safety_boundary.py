...
class ExperimentSafetyBoundary:
    ...
    def evaluate_scenario(
        self, scenario: AdversarialScenario, is_human_approved: bool = False
    ) -> PolicyDecision:
        ...
        if self.constraints.requires_human_approval and not is_human_approved:
            return PolicyDecision(
                organization_id=scenario.organization_id,
                scenario_id=scenario.scenario_id,
    ...
    def evaluate_action_ir(
        self, action: ActionIR, org_id: UUID, current_time: Optional[datetime] = None
    ) -> PolicyDecision:
        ...
        if action.run_as_user not in self.constraints.allowed_users:
            return PolicyDecision(
                organization_id=org_id,
                blueprint_id=action.blueprint_id,
                status=PolicyDecisionStatus.DENIED,
